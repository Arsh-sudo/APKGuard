  ...
  return (
    <div id="report-detail-view" className="min-h-screen py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-2 border-b border-white/[0.08] pb-1 overflow-x-auto">
        {[
          { id: 'ai', label: 'CISO AI Threat Intelligence', icon: Bot, badge: 'Threat Engine' },
          { id: 'ml', label: 'Threat Vector Attribution', icon: Binary, badge: 'Drebin Taxonomy' },
          ...
          {llm_analysis ? (
            <div className="space-y-6">
              ...
              <div className="glass-panel rounded-2xl p-6 border border-white/[0.08] relative overflow-hidden">
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <span className="text-[10px] font-mono text-slate-400">
                        {llm_analysis.model || 'APKGuard Threat Intelligence Engine'}
                      </span>
                    </div>
                  </div>
                ...
                <p className="text-sm text-slate-200 leading-relaxed font-sans bg-white/[0.02] p-4 rounded-xl border border-white/[0.04] whitespace-pre-line">
                  {llm_analysis.executive_summary || llm_analysis.threat_summary || 'No threat summary available.'}
                </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                  <div className="p-4 rounded-xl bg-white/[0.02] border border-white/[0.05]">
                    ...
                    <p className="text-xs text-slate-300 leading-relaxed whitespace-pre-line">
                      {llm_analysis.threat_mechanism || llm_analysis.code_analysis || 'No detailed mechanism provided.'}
                    </p>
                  </div>
                  ...
                  <div className="p-4 rounded-xl bg-white/[0.02] border border-white/[0.05]">
                    ...
                    <p className="text-xs text-slate-300 leading-relaxed whitespace-pre-line">
                      {llm_analysis.plain_english_advisory || llm_analysis.plain_explanation || 'No plain-English advisory available.'}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="glass-panel rounded-2xl p-6 border border-white/[0.08]">
                  ...
                  <div className="space-y-3">
                    {(llm_analysis.high_risk_permissions_context && llm_analysis.high_risk_permissions_context.length > 0) ? (
                      ...
                      <div className="text-xs text-slate-400 italic p-3">
                        {llm_analysis.permission_analysis || 'No excessive or dangerous capabilities flagged in this category.'}
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="glass-panel rounded-2xl p-6 border border-white/[0.08]">
                  ...
                  <div className="space-y-2.5">
                    {(llm_analysis.recommendations && llm_analysis.recommendations.length > 0) ? (
                      ...
                      <div className="text-xs text-slate-300 p-3 rounded bg-white/[0.02] border border-white/[0.04]">
                        {llm_analysis.ciso_recommendation || 'Standard enterprise endpoint monitoring recommended.'}
                      </div>
                    )}
                  </div>
                </div>
      ...
      {/* TAB 2: Threat Vector Attribution */}
      {activeTab === 'ml' && (
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-base font-bold text-white font-sans flex items-center gap-2">
                  <span>Threat Vector Importance & Feature Attribution</span>
                  <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-sky-500/10 text-sky-300 border border-sky-500/20">
                    Drebin Taxonomy
                  </span>
                </h3>
                <p className="text-xs text-slate-400 mt-0.5">
                  Static indicators from permission declarations, API calls, and bytecode signatures driving the verdict score.
                </p>
              </div>
              <div className="text-right">
                <span className="text-xs font-mono text-slate-400 block">Operating Threshold:</span>
                <span className="text-sm font-mono font-bold text-amber-400">τ = 0.80</span>
              </div>
            </div>
